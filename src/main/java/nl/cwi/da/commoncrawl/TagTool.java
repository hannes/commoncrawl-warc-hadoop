	...
	public static class TagToolMapper extends MapReduceBase implements
		...
		public void map(Text key, WritableArchiveRecord value,
			...
			try {
				...
				// TODO: payload here, e.g.
				InputStreamReader inputReader = new InputStreamReader(
						WARCRecordUtils.getPayload(record), cs);
				
				// Alternatively, if you need a correct DOM tree (much slower & crash-prone!)
				Document doctree = Jsoup.parse(
						WARCRecordUtils.getPayload(record), cs.toString(),
						uri.toString());
				
				// TODO: produce more sensible output
				output.collect(new Text(uri.toString()), new Text("42"));
